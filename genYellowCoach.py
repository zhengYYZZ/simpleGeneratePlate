...
class GenYellowCoach:
    ...
    def genBatch(self, batchSize, outputPath, size):
        ...
        for i in tqdm(range(batchSize)):
            ...
            cv2.imwrite(outputPath + "/" + str('Coach') + str(i).zfill(2) + ".jpg", img)
            str_rect = []
            for x, y in zip(self.pointG, plateKey):
                str_rect.append([y, rectangle_vertex(x[0], x[1], x[2], x[3])])
            self.yoloLabelWrite(str_rect, img.shape, outputPath + "/" + str('Coach') + str(i).zfill(2) + ".txt")
            str_rect.clear()
            self.pointG.clear()
